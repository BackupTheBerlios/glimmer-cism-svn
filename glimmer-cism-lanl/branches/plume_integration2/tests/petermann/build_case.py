...
class GCConfig(object):
    ...
    def __init__(self):
        ...
        self.vals = { 'parameters' : { 'geothermal' :   0.0,
                                       ...
                                       # model%paramets%geot
                                       'default_flwa' : None,
                                       # Glen's law A to use in isotherm case 
                                       # model%paramets%default_flwa
                                       'flow_factor' : 1,
                                       # enhancement factor for glen's A 
                                       # model%paramets%flow_factor
                                       'ice_limit' : None,
                                       # min thickness for running ice dynamics
                                       # model%numerics%thklim
                                       'marine_limit' : 0.0,
                                       # When to chop off marine ice
                                       # NB: not used if marine_margin = 0
                                       # is used
                                       # model%numerics%mlimit
                                       'calving_fraction' : 0.0,
                                       # fraction of ice to remove from
                                       # floating ice
                                       ...
                                       # 'basal_tract' :
                                       # 5-value parameters - obsolete
                                       'basal_tract_const' : 0.0,
                                       # basal_tract_const used over base
                                       # model%paramets%btrac_const
                                       # 'basal_tract_max' :
                                       # model%paramets%btrac_max
                                       # 'basal_tract_slope' :
                                       # dependence of basal traction
                                       # on basal water depth
                                       # model%paramets%btrac_slope
                                       # 'stressin':
                                       # initial backstress in areas of
                                       # positive thickness
                                       # NB: only used when marine_margin = 5
                                       # model%climate%stressin
                                       # 'stressout' :
                                       # initial backstress assigned to
                                       # other areas
                      ...
                      'Petermann shelf' : {  'air_temperature' : None,
                                             # Temp assigned to ice in
                                             # isothermal case
                                             'accumulation_rate' : None,
                                             # In meters per year
                                             'eustatic_sea_level' : 0.0,
                                             # Height of sea level relative
                                             # to initial height
                                             },
                      
                      'options' : {'flow_law' : 0,
                                   # flow_law = 2 means constant A 
                                   # model%options%whichflwa
                                   'evolution' : 3,
                                   # evolution = 0 means pseudo-diffusion
                                   #           = 1 means ADI scheme
                                   #           = 2 means iterated diffusion
                                   #           = 3 means LANL incr. remap
                                   #           = 4 upwind advection
                                   # model%options%whichevol
                                   'temperature' : 1,
                                   # temperature = 1 means full 3D
                                   #                 thermal evolution
                                   #             = 0 means set to air
                                   #                 temperature
                                   # model%options%whichtemp
                                   'vertical_integration' : 1,
                                   #vertical_integration = 1 constrained
                                   #                      to obey kinematic BC
                                   # model%options%whichwvel
                                   'marine_margin' : 0,
                                   # marine_margin 
                                   # = 0 means ignore marine margin
                                   #   (no chopping)
                                   # model%options%whichmarn
                                   'topo_is_relaxed' : 1,
                                   # topo_is_relaxed = 1 means the initial
                                   # topography is relaxed
                                   # model%options%whichrelaxed

                                   'slip_coeff' : 1,
                                   # slip_coeff = 0 means set to 0 everywhere
                                   #      = 1 means basal traction is  a
                                   #        constant (basal_tract_const)
                                   # = 2 means Set to (non--zero) constant
                                   #  where where temperature
                                   #  is at pressure melting point of ice,
                                   #  otherwise to zero
	                           ...
	                           'x_invariant' : None,
                                   # = 1 means variables don't change in
                                   #   x direction
                                   # model%options%x_invariant
                                   'diagnostic_run' : 0,
                                   # = 1 makes glide stop after diagnosing
                                   #   velocities
                                   ...
                                   'basal_water' : 3,
                                   # basal_water = 0 means calc from local
                                   # basal water balance
                                   #  = 1 means compute basal water flux
                                   #    then find depth via cal
                                   #  = 2 means no basal water
                                   # model%options%whichbwat

                                   'use_plume' : None,
                                   # use_plume = 0 means use usual
                                   #       Glimmer-CISM method to calc bmlt
                                   #  = 1 means use plume model to calculate
                                   #    bmlt above floating ice
                                   # model%options%use_plume
                                   # 'ioparams' :
                                   #i/o parameters file
                                   },
                      'grid' : { 'sigma_builtin' : 1,
                                 # sigma_builtin = 0 means use the
                                 ...
                                 # model%general%upn
                                 'ewn' : None,
                                 # number of grid nodes in east-west
                                 # model%general%ewn
                                 'nsn' : None,
                                 # number of grid nodes in north-south
                                 # model%general%nsn
                                 'dew' : None,
                      ...
                      'ho_options' : { 'which_ho_sparse_fallback' : -1,
                                       ...
                                       # NB: only used when using Payne-Price
                                       # diagnostic scheme
                                       ...
                                       'diagnostic_scheme' : 3,
                                       # which higher-order diagnostic scheme
                                       # to use:
                                       # = 3 means Payne-Price scheme
                                       # = 2 means Pattyn staggered ?        
                                       # model%options%which_ho_diagnostic
                                       'prognostic_scheme' : 0,
                                       # which higher-order prognostic scheme
                                       # to use
                                       # thickness evolution (only used
                                       # in thick_nonline_evolve
                                       # and thick_lin_evolve, but not
                                       # in incremental remapping)
                                       # model%options%which_ho_prognostic
                                       'include_thin_ice' : 0,
                                       # whether or not to include thin ice
                                       # in HO calculation
                                       # 1 means true
                                       # model%options%ho_include_thinice
                                       'which_ho_efvs' : 0
                                       # ho effective viscosity 
                      ...
                      'CF input' : { 'name' : None,
                                     #name of netcdf file containing input
                                     # data fields
                                     'time' : None
                      ...
                      'CF output' : { 'variables' : ' '.join(['lsurf','usurf',
                                                              ...
                                                              'tau_hom_yz',
                                                              'tau_hom_xy']),
                                      ### NB: there is a (250) character
                                      # limit on line length!!!
                                      
                                      # the following is a list of all
                                      # possible output variables:
                                      # level lithoz x0 x1 y0 y1 acab
                                      # acab_tavg age artm backstress
                                      # beta bheatflx bmlt bmlt_tavg
                                      # btemp btrc bwat bwatflx calving 
                                      # diffu dusrfdtm eus flwa gl_ew
                                      # gl_ns gline_flux iarea ivol
                                      # kinbcmask lat litho_temp lon lsurf
                                      # relx slc soft surfvel tau_xz tau_yz
                                      # taux tauy temp thk thkmask topg ubas
                                      # ubas_tavg uflx usurf 
                                      # uvel uvelhom vbas vbas_tavg velnormhom
                                      # vflx vvel vvelhom wgrd wvel

                                      'frequency' : None,
                                      # time in between writing state to
                                      #output file (in years)
                                      'name' : None,
                                      # name of output file
                                      'start' : None,
                                      'stop'  : None,
                      ...
                      'time' : { 'tstart' : None,
                                 ...
                                 # 'profile' :
                                 # profile period
                                 # model%numerics%profile_period
                                 },
                      'plume' : { 'plume_nl_filename' : None,
                                  # path to plume namelist file
                                  'plume_output_file' : None,
                                  # netcdf file with plume output
                                  'suppress_ascii_output' : True,
                                  #  suppress all old-style ASCII data output
                                  'suppress_logging' : False,
                                  # suppress all screen and file output 
                                  'plume_output_prefix' : None,
                                  # prefix to put on the old style ASCII output
                                  'plume_output_dir' : './',
                                  # where to write the output files
                                  'plume_write_all_states' : False,
                                  # option to write out all timesteps
                                  ...
                                  # plume steadiness tolerance 
                                  # (max relative change in bmelt and speed)
                                  'plume_imax' : None,
                                  'plume_kmin' : None,
                                  'plume_kmax' : None,
                                  'plume_const_bmlt' : False,
                                  # Apply a uniform melt rate under floating
                                  # ice
                                  'plume_const_bmlt_rate' : 0.0
                                  # At given rate in meters per year
                                  }
}
...
def get_config(config_filename):
    ...
    for req_var in req_vars:
        if (not req_var in configVals):
            raise Exception('Did not find %s defined in %s' %
                            (req_var,
                             config_filename))
