...
subroutine glam_velo_fordsiapstr(ewn,      nsn,    upn,  &
  ...
  ! assigning value for nonlinear iteration flag
...
!    if (counter .le. 2) NL_target = NL_tol * L2norm
    if (counter .le. 2) NL_target = 1.0d-4 
...
! intent(inout) btraction, calcs rhsd and g within
...
! intent(out) for valueset is rhsd 
        end do
...
!***********************************************************************
  ...
  function distributed_globalID_to_localindex(globalID)
  	! distributed_globalID_to_localindex searches loc2_array(:,:,1) for the globalID closest to the 
  	...
  	! Original implementation using myIndices, and then fast inverse, by JEFF 11/2010 and 11/2011
        ! Current loc2_array by PW 12/2011
	  use parallel
      ...
      ! Fast Inverse
      !JEFF lindex = GlobalIDsGet(globalID)  ! C++ hash table for fast inverse
      !JEFF 
      !JEFF if (lindex .ne. 0) then
      !JEFF     ! match found
      !JEFF     distributed_globalID_to_localindex = lindex
      !JEFF     return
      !JEFF else
      !JEFF    ! If get to here, then no match found.  Return -1 to cause bounds check error.
      !JEFF    write(*,*) "Error in distributed_globalID_to_localindex().  GlobalID to match = ", globalID
      !JEFF    write(*,*) "MyIndices vector = "
      !JEFF    write(*,*) myIndices
      !JEFF 
      !JEFF    distributed_globalID_to_localindex = -1
      !JEFF    return
      !JEFF endif
#else
