...
!JEFF Trying without the pointer attribute    integer, dimension(:,:),pointer       :: mask    !*FD grid type mask
  ...
  dew)
    use glide_mask
    implicit none
!JEFF pointer was causing me problems, so try taking out
...
!JEFF Trying without pointer attribute     integer, dimension(:,:),pointer :: mask    !*FD grid type mask
     integer, dimension(:,:) :: mask    !*FD grid type mask
